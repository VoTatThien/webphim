...
<div class="booking-info-bar" style="background: #232023; border: 1px solid #4a3e43; padding: 20px; margin: 20px auto; max-width: 1200px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
    <div style="display: flex; flex-wrap: wrap; justify-content: space-around; align-items: center; color: white;">
        ...
        <?php
        // ====================================================================
        // DECISION TREE RECOMMENDATION ENGINE - GỢI Ý COMBO THÔNG MINH
        // ====================================================================
        // Thuật toán: Decision Tree (ID3 - Information Gain)
        // Dựa trên: Thể loại phim, Giờ chiếu, Khoảng tuổi, Giới tính,
        //           Thứ trong tuần, Số ghế
        // ====================================================================
        
        // Include model Decision Tree & Tracking
        ...
        // ID người dùng và phim
        $user_id = $_SESSION['user']['id'] ?? 0;
        $id_phim = $_SESSION['tong']['id_phim'] ?? 0;
        $gio_chieu = $_SESSION['tong']['thoi_gian_chieu'] ?? '19:00';
        
        // --- BƯỚC 2: Gọi Decision Tree để gợi ý combo ---
        
        $reco_result = recommend_combo($user_id, $id_phim, $gio_chieu, $so_ghe, $combos ?? []);
        
        $suggested_combo = $reco_result['suggested_combo'];
        $reco_title = $reco_result['reco_title'];
        $reco_desc_label = $reco_result['reco_desc'];
        $reco_type = $reco_result['reco_type'];
        $confidence = $reco_result['confidence'];
        
        // --- BƯỚC 3: Ghi log Recommendation (Tracking) ---
        $reco_log_id = 0;
        if ($suggested_combo) {
            try {
                $reco_log_id = reco_log_suggestion(
                    ...
                    $confidence,
                    [
                        'features'  => $reco_result['features'] ?? [],
                        'tree_path' => $reco_result['tree_path'] ?? '',
                        'all_reco'  => $reco_result['all_recommendations'] ?? [],
                    ]
                );
        ...
        <div class="recommendation-section">
            <div class="recommendation-badge"><i class="fa fa-fire"></i> <?= __("AI Khuyên dùng") ?></div>
