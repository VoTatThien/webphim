...
if(isset($_GET['act']) && $_GET['act']!=""){
    ...
    switch ($act) {
        ...
        include "view/login/dangnhap.php";
            ...
            case "dangky": //đăng ký tk
                ...
                if (isset($_POST['dangky']) && $_POST['dangky'] != "") {
                    ...
                    $pass = $_POST['pass'];
                    $email = $_POST['email'];
                    $ngay_sinh = null; // Không còn dùng ngày sinh, thay bằng khoảng tuổi
                    $gioi_tinh = !empty($_POST['gioi_tinh']) ? $_POST['gioi_tinh'] : null;
                    $khoang_tuoi = !empty($_POST['khoang_tuoi']) ? $_POST['khoang_tuoi'] : null;
                    ...
                    if (
                        ...
                        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            ...
                            if ($email_check && $email_check['email'] == $email) {
                                ...
                                // Thêm tài khoản mới
                                insert_taikhoan($email, $user, $pass, $name, $sdt, $dc, $ngay_sinh, $gioi_tinh, $khoang_tuoi);
        ...
        case "datve2": //Đặt vé chọn ghế
            ...
            // KIỂM TRA ĐĂNG NHẬP - Bắt buộc đăng nhập để đặt vé
            if (isset($_SESSION['user']) && ($_SESSION['user'])) {
                // Đã đăng nhập → Hiển thị trang chọn ghế
                include "view/dv2.php";
            } else {
                // Chưa đăng nhập → Chuyển hướng về trang đăng nhập
                $_SESSION['redirect_after_login'] = 'index.php?act=datve2&id=' . ($_GET['id'] ?? '');
                $thongbao = "Vui lòng đăng nhập hoặc đăng ký tài khoản để đặt vé xem phim.";
                include "view/login/dangnhap.php";
            }
            break;
        ...
        case 'xacnhan': //Thanh toán thành công
            ...
            if (isset($_SESSION['id_hd']) && $_SESSION['id_hd'] > 0) {
                ...
                if (isset($_SESSION['user']) && $_SESSION['user']) {
                    ...
                    // Chỉ cộng điểm nếu chưa cộng điểm cho đơn hàng này (kiểm tra session)
                    $da_cong_diem_key = 'da_cong_diem_' . $_SESSION['id_hd'];
                    
                    if (isset($_SESSION['user']) && $load_ve_tt && !isset($_SESSION[$da_cong_diem_key])) {
