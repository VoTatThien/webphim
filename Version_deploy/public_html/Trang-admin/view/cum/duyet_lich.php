...
<div class="content-body">
    ...
    <div class="card">
        <!-- Bulk action toolbar -->
        <div id="bulkActionToolbar" style="display: none; padding: 15px 20px; background-color: #f8f9fa; border-bottom: 1px solid #dee2e6;">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <span class="text-muted">
                        <i class="zmdi zmdi-info"></i> 
                        Đã chọn: <strong id="selectedCount">0</strong> kế hoạch
                    </span>
                </div>
                <div class="col-md-6 text-right">
                    <button type="button" class="btn btn-sm btn-success" onclick="approveBulk()" title="Duyệt tất cả kế hoạch được chọn">
                        <i class="zmdi zmdi-check-all"></i> Duyệt tất cả
                    </button>
                    <button type="button" class="btn btn-sm btn-danger" onclick="rejectBulk()" title="Từ chối tất cả kế hoạch được chọn">
                        <i class="zmdi zmdi-close-circle"></i> Từ chối tất cả
                    </button>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="clearSelection()" title="Bỏ chọn tất cả">
                        <i class="zmdi zmdi-close"></i> Bỏ chọn
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead class="thead-light">
                        <tr>
                            <th class="border-0" style="width: 50px; text-align: center;">
                                <input type="checkbox" id="selectAllCheckbox" onchange="toggleSelectAll(this)" title="Chọn/bỏ chọn tất cả">
                            </th>
                    ...
                    <tbody>
                        <?php if (!empty($ds_lich)): ?>
                            <?php foreach ($ds_lich as $r): ?>
                                ...
                                ?>
                                <tr class="kehoach-row" data-ma-ke-hoach="<?= htmlspecialchars($r['ma_ke_hoach']) ?>">
                                    <td style="text-align: center;">
                                        <?php if ($st_norm === 'cho_duyet'): ?>
                                            <input type="checkbox" class="kehoach-checkbox" value="<?= htmlspecialchars($r['ma_ke_hoach']) ?>" onchange="updateBulkToolbar()">
                                        <?php endif; ?>
                                    </td>
                                    <td>
...
function toggleSelectAll(checkbox) {
    // Toggle all checkboxes khi click "Select All"
    const checkboxes = document.querySelectorAll('.kehoach-checkbox');
...
function updateBulkToolbar() {
    ...
    const checkboxes = document.querySelectorAll('.kehoach-checkbox:checked');
    const toolbar = document.getElementById('bulkActionToolbar');
    const selectAllCheckbox = document.getElementById('selectAllCheckbox');
    const totalCheckboxes = document.querySelectorAll('.kehoach-checkbox').length;
...
function clearSelection() {
    // Bỏ chọn tất cả
    document.querySelectorAll('.kehoach-checkbox').forEach(cb => {
        cb.checked = false;
    });
...
function approveBulk() {
    // Duyệt tất cả kế hoạch được chọn
    const checkboxes = document.querySelectorAll('.kehoach-checkbox:checked');
    if (checkboxes.length === 0) {
    ...
    if (!confirm(`Bạn chắc chắn muốn duyệt ${checkboxes.length} kế hoạch chiếu này?`)) {
        return;
    }
    
    // Collect selected ma_ke_hoach
    const selected = Array.from(checkboxes).map(cb => cb.value);
    
    // Send to backend
    $.ajax({
        type: 'POST',
        url: 'index.php?act=bulk_duyet_lichchieu',
        data: {
            action: 'duyet',
            ma_ke_hoach_list: selected
        },
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                alert(`Duyệt thành công ${response.count} kế hoạch`);
                location.reload();
            } else {
                alert('Lỗi: ' + (response.message || 'Không thể duyệt kế hoạch'));
            }
        },
        error: function() {
...
function rejectBulk() {
    // Từ chối tất cả kế hoạch được chọn
    const checkboxes = document.querySelectorAll('.kehoach-checkbox:checked');
    if (checkboxes.length === 0) {
        alert('Vui lòng chọn ít nhất 1 kế hoạch');
        return;
    ...
    if (!confirm(`Bạn chắc chắn muốn từ chối ${checkboxes.length} kế hoạch chiếu này?`)) {
        return;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                alert(`Từ chối thành công ${response.count} kế hoạch`);
                location.reload();
            } else {
                alert('Lỗi: ' + (response.message || 'Không thể từ chối kế hoạch'));
            }
        },
        error: function() {
            alert('Lỗi khi gửi yêu cầu. Vui lòng thử lại');
